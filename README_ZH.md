...
| 输入                   | 描述                                                                                          | 是否必需 | 默认值               |
| ---------------------- | --------------------------------------------------------------------------------------------- | -------- | -------------------- |
| `project-path`         | 一个或多个 `.csproj` 文件的路径，每行一个。如果未指定，则自动检测 `.csproj` 文件。            | 否       | `''`                 |
| `exclude-patterns`     | 要从扫描中排除的额外目录模式（每行一个）。默认排除项：`Test`、`test`、`node_modules`、`.venv` | 否       | `''`                 |
| `max-depth`            | 扫描 `.csproj` 文件的最大深度                                                                 | 否       | `'5'`                |
| `mod-folder-name`      | 模组文件夹的自定义名称。输出路径：`SPT/user/mods/{模组文件夹名称}/`                           | 否       | `''`（使用项目名称） |
| `resource-paths`       | 要包含的额外资源目录（每行一个）。默认值：`db`、`data`、`wwwroot`、`res`                      | 否       | `''`                 |
| `include-source-files` | 是否在打包中包含源代码文件（`.cs`、`.ts` 等）                                                 | 否       | `'false'`            |

## 输出参数

| 输出              | 描述                                          |
| ----------------- | --------------------------------------------- |
| `artifact-name`   | 上传的工件名称（多个项目时为 JSON 数组）      |
| `artifact-path`   | 生成的 zip 文件路径（多个项目时为 JSON 数组） |
| `project-name`    | 打包的项目名称（多个项目时为 JSON 数组）      |
| `project-version` | 打包的项目版本号（多个项目时为 JSON 数组）    |
